...
namespace NDepend.PowerTools {
   class Program {
...
#if NETCORE
         $@"..{System.IO.Path.DirectorySeparatorChar.ToString()}Lib"
#else
         @".\Lib"
#endif
         );
      ...
      static void MainSub() {
         // 22Sept2015: Increase the buffer height to the maximum. 
         //             Because once reached some height computation algorithm will be broken.
         //             Like this bug mentionned in this post in the section PowerTools: http://www.newventuresoftware.com/blog/ndepend-5-4-first-impressions/
         // 13Oct2021: Console.set_BufferHeight not available on Linux/MacOS
         if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) {
            Console.BufferHeight = Int16.MaxValue - 1;
         }

         var powerTools = new IPowerTool[] {
                  new TechnicalDebt.TechnicalDebtPowerTool(),
                  ...
                  new DotNetCoreAPIChanges.DotNetCoreAPIChangesPowerTool(),
         };
         ...
         while (true) {
            ...
            Console.WriteLine("Power Tools aim at demonstrating NDepend API syntax and capabilities.");
            Console.WriteLine();
            ...
            while (true) {
               ...
               char keyChar = Char.ToLower(Console.ReadKey().KeyChar);
               Console.WriteLine("");
            ...
            IPowerTool powerTool = powerTools[powerToolIndex];

            if (!OSHelper.IsOnWindows && !powerTool.AvailableOnLinuxMacOS) {
               Console.ForegroundColor = ConsoleColor.Red;
               Console.WriteLine($@"Power tool {{{powerTool.Name}}} is not available on Linux or MacOS.
Type a key to try another Power Tool.");
               Console.ReadKey();
               continue;
            }
